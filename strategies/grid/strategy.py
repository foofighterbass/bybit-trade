...
class GridStrategy(BaseStrategy):
    def __init__(self, strategy_id: str, params: dict):
        ...
        self.levels      = int(params["levels"])
        self.spacing     = float(params["spacing_pct"]) / 100
        self.qty         = str(params["qty"])
    ...
    def setup(self, reset: bool = False) -> None:
        ...
        saved = database.load_active_orders(self.id, self.symbol)
        if saved and not reset:
            self.log.info("Восстановлено %d ордеров из БД", len(saved))
            self._orders = {r["order_id"]: {"price": r["price"], "side": r["side"]} for r in saved}
            return

        center = float(exchange.get_ticker(self.symbol)["lastPrice"])
        self.log.info("Инициализация сетки | %s | центр=%.1f | уровней=%d | шаг=%.2f%%",
    ...
    def tick(self) -> None:
        ...
        for order_id, info in filled:
            ...
            self.log.info("Исполнен %s @ %.1f%s", side, price,
                          f"  pnl={pnl:+.4f}" if pnl else "")
            database.mark_filled(self.id, order_id, pnl or 0)
            database.log_trade(self.id, self.symbol, side, float(self.qty), price, order_id)

            counter_side  = "Sell" if side == "Buy" else "Buy"
            counter_price = round(
                price * (1 + self.spacing if side == "Buy" else 1 - self.spacing), 1
            )
    ...
    def _place(self, side: str, price: float) -> None:
        try:
            ...
            database.save_order(self.id, oid, self.symbol, side, price, self.qty)
            self.log.debug("Размещён %s @ %.1f", side, price)
        except Exception as exc:
            self.log.error("Ошибка размещения %s @ %.1f: %s", side, price, exc)
